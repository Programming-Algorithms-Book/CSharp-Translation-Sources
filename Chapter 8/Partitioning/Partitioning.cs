...
{
    private static int[] s = { 0, 23, 15, 89, 170, 25, 1, 86, 80, 2, 27 }; /* Редица (нулевият елемент не се ползва) */
    private static readonly int n = s.Length - 1; /* Брой елементи в редицата */
    private const int k = 4; /* Брой групи */
    private static long[] p = new long[n + 1]; /* Префиксни суми */
    private static long[,] f = new long[n + 1, n + 1]; /* Целева функция */
    private static long[,] b = new long[n + 1, n + 1]; /* За възстановяване на решението */
